...
assert.match(staging,/region-analyzer-viewer-v001\.js\?v=008/,'staging must load the Region Analyzer viewer');
assert.match(staging,/RA v008/,'staging must expose the Region Analyzer build version');
...
assert.match(viewer,/function initRegionColors\(m\)\{regionColors=new Map\(Object\.keys\(m\?\.region_dictionary\|\|\{\}\)\.map\(id=>\[id,COLORS\[colorIndex\(id\)\]\]\)\);\}/,'viewer must freeze a manifest-level region identity color table');
assert.match(viewer,/function regionColor\(id\)\{return regionColors\.get\(id\)\|\|COLORS\[colorIndex\(id\)\];\}/,'region color lookup must come from the fixed manifest table');
