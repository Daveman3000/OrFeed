...
These are descriptive calibration results from previously inspected surfaces, not fresh validation of economic thresholds or predictive performance. No Step 4 mask, RR anchor membership artifact, or implementation is claimed to be materialized yet. A later executable run must bind its policy exact-byte hash, descriptor and package hashes, surface policy, cleaned-domain identity, topology version, and sorted `analysis_key` membership of every anchor before Step 5 uses it.
