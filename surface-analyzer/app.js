...
let meta,currentKey='r_per_trade',values=null,cdf=null;
const cache=new Map();
...
const groups=[{s:0,e:30,n:'Fixed · 1 trade',mode:'Fixed',trades:1},{s:30,e:60,n:'Fixed · 2 trades',mode:'Fixed',trades:2},{s:60,e:65,n:'Trail · 1 trade',mode:'Trail',trades:1},{s:65,e:70,n:'Trail · 2 trades',mode:'Trail',trades:2},{s:70,e:160,n:'Hybrid · 1 trade',mode:'Hybrid',trades:1},{s:160,e:250,n:'Hybrid · 2 trades',mode:'Hybrid',trades:2}];
...
function displayQ(q){const m=meta[currentKey];let z=viewSel.value==='pct'?Math.round(cdf[q]*15):q;if(m.invert)z=15-z;return z;}
...
function rawApprox(q){const m=meta[currentKey];return m.lo+(m.hi-m.lo)*(q/15);}
canvas.addEventListener('mousemove',e=>{if(!values)return;const r=canvas.getBoundingClientRect(),col=Math.max(0,Math.min(COLS-1,Math.floor((e.clientX-r.left)/r.width*COLS))),row=Math.max(0,Math.min(ROWS-1,Math.floor((e.clientY-r.top)/r.height*ROWS))),q=values[row*COLS+col],m=meta[currentKey],o=outerInfo(row),orig=meta.__order__[col],inn=innerInfo(orig),v=rawApprox(q),pct=Math.round(cdf[q]*100),target=inn.mode==='Trail'?'30m OTF trail':`Rule ${inn.rule} · ${inn.multiple.toFixed(2)}× London`;hover.innerHTML=`<span>Outer: <b>${row+1} / 224</b></span><span>Inner: <b>${orig+1} / 250</b></span><span>Metric: <b>${m.label}</b></span><span>Range: <b>${o.mode} · ${o.threshold}</b></span><span>London close > Asia: <b>${o.londonClose?'Yes':'No'}</b></span><span>PM close: <b>${o.pmLabel}</b></span><span>Entry location: <b>${o.entry}</b></span><span>Management: <b>${inn.mode} · ${inn.trades} trade${inn.trades===1?'':'s'}</b></span><span>Stop: <b>${inn.stop}% London</b></span><span>Allocation: <b>${inn.allocation==null?'—':inn.allocation+'%'}</b></span><span>Target: <b>${target}</b></span><span>Value: <b>${fmt(v,m.decimals)}${viewSel.value==='pct'?` · P${pct}`:''}</b></span>`;});
metricSel.addEventListener('change',()=>setMetric(metricSel.value));
viewSel.addEventListener('change',draw);
addEventListener('resize',size);
(async function init(){try{meta=await fetch('metrics.json',{cache:'no-store'}).then(r=>r.json());for(const [k,m] of Object.entries(meta)){if(k.startsWith('__'))continue;const opt=document.createElement('option');opt.value=k;opt.textContent=m.label;metricSel.appendChild(opt);}metricSel.value=currentKey;size();await setMetric(currentKey);}catch(e){loading.textContent='Could not initialize interface: '+e.message;}})();