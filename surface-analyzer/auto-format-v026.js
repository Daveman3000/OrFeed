(function(root){
  ...
  function remap(surface,xOrder,yOrder,original,autoInfo){
    ...
    const xr=new Map(x.map((v,i)=>[v.sig,i])),yr=new Map(y.map((v,i)=>[v.sig,i])),metrics={},supportFields={},params={},physicalIndexByVisual=surface.physicalIndexByVisual?new Int32Array(n):null,seen=new Uint8Array(n);
    ...
    for(let old=0;old<n;old++){
      ...
      if(physicalIndexByVisual)physicalIndexByVisual[pos]=surface.physicalIndexByVisual[old];
    }
    ...
    return {...surface,rows,cols,metrics,supportFields,semanticParameterIndices:params,semanticAxis,semanticDescriptor:descriptor,physicalIndexByVisual:physicalIndexByVisual||surface.physicalIndexByVisual,autoFormatOriginalLayout:original,autoFormat:autoInfo};
  }
