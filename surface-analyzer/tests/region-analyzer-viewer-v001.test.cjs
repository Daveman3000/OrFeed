...
assert.match(staging,/region-analyzer-viewer-v001\.js\?v=011/,'staging must load the Region Analyzer viewer');
assert.match(staging,/RA v011/,'staging must expose the Region Analyzer build version');
...
assert.match(viewer,/im\.data\[px\+3\]=78/,'region fill opacity must be uniform');
